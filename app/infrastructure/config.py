...
Every other module receives values from Settings via dependency injection.
"""
...
class Settings(BaseSettings):
    ...
    Pydantic raises a descriptive ValidationError at import time if any
    required variable is absent — no silent startup with missing config.
    """
    ...
    max_queue_size: int = Field(
        ...
        description="Max queued songs per chat",
    )
    default_volume: int = Field(
        default=DEFAULT_VOLUME,
        description="Default playback volume (0-200)",
    )

    # ── Voice Chat ────────────────────────────────────────────────────────────
    # Optional Pyrogram string session for a user assistant account.
    # When set, the assistant joins voice chats (required for audio production).
    # When absent, the bot itself joins (requires Manage Voice Chats admin perm).
    ...
    # Required for age-restricted / region-locked videos.
...
def get_settings() -> Settings:
    ...
    Calling this at startup gives a clear error if any required variable
    is missing, rather than a cryptic AttributeError during operation.
    """
    try:
